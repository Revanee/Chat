...
public class TokenManager {
    ...
    public static void addToken(String user) {
        ...
        if(!user.isEmpty()) tokens.add(new TokenManager().new Token(user));
    }
    
    public static String getUserFromToken(String reqToken) {
        String user = null;
        for (Token itToken : tokens) {
            if (reqToken.equals(itToken.token)) user = itToken.user;
        }
        return user;
    }
    ...
    public static String getTokenFromUser(String reqUser) {
        ...
        for (Token itToken : tokens) {
            if (reqUser.equals(itToken.user)) token = itToken.token;
        }
    ...
    public static void removeToken(String token_str) {
        ...
        if(deleteCandidate != null) tokens.remove(deleteCandidate);
    }
